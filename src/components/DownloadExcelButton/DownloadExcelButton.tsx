...
export const DownloadExcelButton = ({ authorId }: Props) => {
  const downloadFile = async () => {
    try {
      const blob = await request.blob(
        `/export/author-vak-excel?authorid=${authorId}`
      );
      ...
      a.href = url;
      a.download = `author_${authorId}_vak.xlsx`;
  ...
  return (
    <button className={styles.downloadExcelButton} onClick={downloadFile}>
      <ExcelSVG />
      <div className={styles.downloadExcelButtonDelimiter} />
      <div className={styles.downloadExcelButtonText}>Скачать</div>
    </button>
  );
};
